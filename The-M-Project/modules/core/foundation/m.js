...
/**
 * Overwrites clear() of LocalStorage to clear only key-value pairs belonging to the application.
...
if(localStorage){
	Object.getPrototypeOf(localStorage).clear = function(param) {
	    var l = this.length;
	    if(param === 'f') { /* Call localStorage.clear() with parameter 'f' to use system wide localStorage.clear() */
            for (var i = l - 1; i >= 0; i--){
                var k = this.key(i);
